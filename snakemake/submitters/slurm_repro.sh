...
snakemake -j 100 --nolock --rerun-incomplete --cluster-config ../cluster_jsons/repro.json --cluster "sbatch -A compbio -p {cluster.partition} -n {cluster.nodes} -t {cluster.time} -J {cluster.J} -o {cluster.o} -e {cluster.e} --mem {cluster.mem}" -s Snakefile_repro
