...
cd ${SUBMITDIR}
sbatch --account=compbio --time=3-01:00:00 --job-name=subber_ASD_mutex_repro --output=${LOGDIR}submitter_repro_%j.out --error=${LOGDIR}submitter_repro_%j.err --partition=long_jobs slurm_repro.sh
